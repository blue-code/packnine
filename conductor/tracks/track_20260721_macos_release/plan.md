...
- [ ] `macos-latest` CI 매트릭스에서 pytest 전체 통과
- [ ] macOS에서 PyInstaller 빌드한 `.app`이 실행되어 압축/해제 왕복(round-trip) 수동 검증 완료
- [ ] 우클릭 메뉴/MoTW 미지원 상태에서도 크래시 없이 정상 동작(예외 미노출) 확인
- [ ] GitHub Release에 macOS 배포물(`.dmg` 또는 zip) 첨부
- [ ] README에 macOS 설치/실행/RAR 의존성 안내 추가
