...
## 8. 게시 (2026-07-21, 같은 날 후속 작업)

가장 최근 성공한 `build-macos` CI 실행(커밋 `78a611a`)의 아티팩트를 내려받아 기존
`v0.6.0` GitHub Release(Windows `PackNine-Setup.exe`/`PackNine.exe`가 이미 첨부된
릴리스)에 `PackNine-macOS-arm64.zip`으로 추가 첨부했다. 별도 태그/버전을 새로 만들지
않은 이유는 이번 트랙이 기능 변경 없이 배포 플랫폼만 추가했기 때문이다(버전 문자열은
`pyproject.toml`의 `0.6.0` 그대로 유지). 릴리스 노트에도 macOS 베타 섹션(Gatekeeper
허용 절차, RAR 의존성, 미지원 기능 안내)을 추가했다.

plan.md 6장 성공 기준 5개 항목을 모두 충족해 `track_status: approved`로 전환한다.
GUI 실사용 검증(사람이 직접 화면을 띄워 확인), 코드 서명/Notarization, Finder Quick
Action 등은 6장/7장에 남긴 대로 후속 트랙 범위다.
